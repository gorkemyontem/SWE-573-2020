...
def one_time_schedules(): 
    ...
    Schedule.objects.create(func='scraper.tasks.find_submission_without_comment', schedule_type=Schedule.CRON, cron = '0 */3 * * *')
    Schedule.objects.create(func='analyser.tasks.polarity_analysis_submission_task', schedule_type=Schedule.CRON, cron = '0 */5 * * *')
    Schedule.objects.create(func='analyser.tasks.polarity_analysis_comment_task', schedule_type=Schedule.CRON, cron = '0 */7 * * *')
...
def polarity_analysis_submission_task(range_limit = 1000, loop_limit = 5):
    for _ in range(range_limit):
        bulk = []
...
def polarity_analysis_comment_task(range_limit = 3000, loop_limit = 5):
    for _ in range(range_limit):
        bulk = []
